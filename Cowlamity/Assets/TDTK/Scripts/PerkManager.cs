...
namespace TDTK {
	...
	public class PerkManager : MonoBehaviour {
		...
		[HideInInspector] public List<int> availableIDList=new List<int>(); 	//ID list of perk available for this level, modified in editor, only used in runtime
		
		public List<int> purchasedIDList=new List<int>(); 		//ID list of perk pre-purcahsed for this level, modified in editor
		private List<Perk> perkList=new List<Perk>();			//actual perk list, filled in runtime based on availableIDList
		...
		public void Init(){
			...
			for(int i=0; i<perkList.Count; i++){
				while(perkList[i].cost.Count<rscCount) perkList[i].cost.Add(0);
				while(perkList[i].cost.Count>rscCount) perkList[i].cost.RemoveAt(perkList[i].cost.Count-1);
				
				while(perkList[i].valueRscList.Count<rscCount) perkList[i].valueRscList.Add(0);
				while(perkList[i].valueRscList.Count>rscCount) perkList[i].valueRscList.RemoveAt(perkList[i].valueRscList.Count-1);
			}
			...
			for(int i=0; i<perkList.Count; i++){
				if(purchasedIDList.Contains(perkList[i].ID)) _PurchasePerk(perkList[i], false);
			}
		...
		public void LoadPurchasedPerk(){
			...
			int count=PlayerPrefs.GetInt("TDTK_UnlockedPerkCount", 0);
			for(int i=0; i<count; i++){
				int perkID=PlayerPrefs.GetInt("TDTK_UnlockedPerk_ID_"+i, -1);
		...
		public string _PurchasePerk(Perk perk, bool useRsc=true){
			...
			if(!purchasedIDList.Contains(perk.ID)) purchasedIDList.Add(perk.ID);
			SavePurchasedPerk();
...
//modifiers goes here		
		...
		public static List<UnitTower> GetUnlockedTowerList(){ return instance==null ? new List<UnitTower>() : instance.unlockedTowerList; }
		public static List<Ability> GetUnlockedAbilityList(){ return instance==null ? new List<Ability>() : instance.unlockedAbilityList; }
		public static List<FPSWeapon> GetUnlockedWeaponList(){ return instance==null ? new List<FPSWeapon>() : instance.unlockedWeaponList; }
		...
		public static List<float> GetRscRegen(){ return instance==null ? new List<float>() : new List<float>( instance.rscRegen ); }
		public static List<float> GetRscGain(){ return instance==null ? new List<float>() : new List<float>(instance.rscGain ); }
		public static List<float> GetRscCreepKilled(){ return instance==null ? new List<float>() : new List<float>(instance.rscCreepKilledGain ); }
		public static List<float> GetRscWaveCleared(){ return instance==null ? new List<float>() : new List<float>(instance.rscWaveClearedGain ); }
		public static List<float> GetRscTowerGain(){ return instance==null ? new List<float>() : new List<float>(instance.rscRscTowerGain ); }
